...
class LossHistory():
    def __init__(self, log_dir, model, input_shape):
        ...
        # Optionally log the model structure
        # try:
...
class EvalCallback():
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        """Generate prediction result files for mAP evaluation."""
        f = open(os.path.join(map_out_path, "detection-results/"+image_id+".txt"), "w", encoding='utf-8') 
        image_shape = np.array(np.shape(image)[0:2])
        # Convert image to RGB format to avoid errors during prediction
        image = cvtColor(image)
        # Add padding to maintain aspect ratio during resizing
        ...
        # Sort and limit results to the top 100 predictions
    ...
    def on_epoch_end(self, epoch, model_eval):
        ...
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for annotation_line in tqdm(self.val_lines):
                ...
                # Generate ground truth result files
